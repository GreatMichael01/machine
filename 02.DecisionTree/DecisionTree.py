...
def splitDataSet(dataSet, axis, value):
    ...
    for featVec in dataSet:
        ...
        if featVec[axis] == value:
            ...
            '''
            reducedFeatVec.extend(featVec[axis+1:])     #expend接受一个列表的参数例如：[1,2].extend([1,2,3])---》[1,2,1,2,3]
            # 收集结果值 axis列为value的行【该行需要排除axis列】
            retDataSet.append(reducedFeatVec)       #append接受一个对象的参数例如：[1,2].append([1,2,3])---》[1,2,[1,2,3]]
    return retDataSet
...
def chooseBestFeatureToSplit(dataSet):
    ...
    # iterate over all the features     #迭代所有功能
    for i in range(numFeatures):
        # create a list of all the examples of this feature     #创建该功能的所有示例的列表
        # 获取每一个feature的list集合
        featList = [example[i] for example in dataSet]
        # get a set of unique values        #获得一组唯一的值
        # 获取剔重后的集合
